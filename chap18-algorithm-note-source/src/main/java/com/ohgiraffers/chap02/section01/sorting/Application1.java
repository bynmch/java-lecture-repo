 ...
 *   "버블링(이동)" 된다.
 *   시간복잡도는 O(n^2)이라 효율적이지 않다.(feat. 데이터 크기가 10000이상이면 위험)
* */
...
//        System.out.println(Arrays.toString(arr));
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(System.in))) {
            int length = Integer.parseInt(br.readLine());   //가급적 한 줄을 문자열로 읽어 숫자로 바꿀 때는
                                                            //Integer.valueOf 대신 Integer.parseInt를 사용
            /* 설명. 한 줄 문자열 파싱하기 */
            /* 설명. 1. split 활용: 정규표현식 해석으로 인해 StringTokenizer보다 시간이 약간 더 걸림*/
...
//            }
            ...
            /* 설명. 2. StringTokenizer 활용: 가장 속도가 빠름 */
...
//            int[] arr = intStream.toArray();
    ...
    public static void solution(int length, int[] arr) {
        for (int i = 0; i < length; i++) {
            ...
            /* 설명. 둘 씩 쌍을 지어 하나의 버블을 끝까지 한 번 보내는 작업 */
            for (int j = 0; j < length - 1; j++) {              //오름차순을 위해 오른쪽이 작으면 swap
                if (arr[j] > arr[j + 1]) {
...
/* 필기.
 *  버블 정렬: 둘 씩 짝지어 더 큰 것을 오른쪽으로 swap
 *
* */
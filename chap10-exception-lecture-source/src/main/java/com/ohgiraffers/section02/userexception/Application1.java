...
public class Application1 {
    ...
    public static void main(String[] args) {
         ...
         *  (try블럭 안에서 발생하는 예외 타입의 종류만큼 가능, 각 상황별로 예외 처리를 달리 할 수 있다.)
         *  try블럭 안에서 예외가 발생하면 이후 try블럭을 건너뛰고 catch 블럭을 순차적으로 타입 체크하며
         *  발생한 예외 타입과 일치하는 catch블럭이 동작함
         *  맨 위에 두면 최상위(조상 클래스)의 catch 블럭만 실행되기 때문에
         *  자식 타입의 예외 타입을 가진 catch블럭 일수록 위쪽에 작성하고 부모 타입
         *  일수록 아래쪽에 작성해야 한다.
        * */
        ExceptionTest et = new ExceptionTest();
        
        /* 설명. 예외 처리를 하면 우리가 원하는 대로 진행됨(프로그램 종료 안되게, 그 외에 다양하게) */
        try {
...
//            System.out.println("너 빚쟁이 아니면 돈이 부족한 걸?");
//        }
        catch (Exception e) {

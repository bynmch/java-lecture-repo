 ...
 * 설명.
 *  reduce()의 3가지 방법
 *  1. one param: Optional 반환(빈 스트림 고려)
 *  2. Two params: 초기값 사용, 직접 값 변환
 *  3. Three params: 병렬 처리 가능
* */
...
public class Application2 {
    ...
    public static void main(String[] args) {
        ...
        /* 설명. 2. 초기값ㅇ르 가진 reduce: 1개의 초기값, 1개의 람다식 */
        ...
        /* 설명. 세 번째로 전달된 람다식은 병렬 스트림일 때만 활용하는 CPU의 여러 코어로 계산된 결과를 결합할 용도의 람다식이다. */
//        Integer reduceTreeParam = Stream.of(4, 5, 1, 2, 3, 9, 6, 7, 8, 10)
//                .parallel()                     //주석을 통해 순차 스트림과 병렬 스트림을 모두 텡스트 해 볼 수 있음
//                .reduce(0,                      //초기값은 parallel일 경우는 요소 갰수만큼 적용
//                        (x, y) -> {
//                            System.out.printf("d%, d% \n", x, y);
//                            return x + y;
//                        },
...
//        System.out.println("최종 결과: " + reduceTreeParam);
        ...
        /* 설명. 단순 연결 */
        String simpleJoin = words.stream().reduce("", (a, b) -> a + b);
        System.out.println("simpleJoin = " + simpleJoin);

        /* 설명. 구분자를 사용한 연결 */
        String joinWithSeparator = words.stream().reduce("", (a, b) -> a.isEmpty() ? b : a + " | " + b);

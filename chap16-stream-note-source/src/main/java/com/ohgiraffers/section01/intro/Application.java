 ...
 *  - 데이터를 필터링, 변환, 통계/집계 등의 작업을 간결하고 효율적으로 처리가능
...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        /* 설명.
         *  Arrays.asList로 생성된 list는 실제로는 java.util.Arrays 내 ArrayList이다.
         *  - 컬렉션의 ArrayList와 다른점
         *  1. 크기 변경 불가(추가 및 삭제 불가)
         *  2. 실제 ArrayList의 소속이 다름
        * */
        List<String> studyTopics = Arrays.asList("Java 기초", "객체지향", "컬렉션", "Stream API", "Spring Boot");

        /* 설명. 1. for-each문 */
        for (String topic: studyTopics) { processTopic(topic); }
        System.out.println();
...
//        studyTopics.stream().forEach(topic -> processTopic(topic));
        ...
        studyTopics.parallelStream().forEach(Application::processTopic);
        System.out.println();
    }

    /* 설명. 현재 실행 중인 쓰레드의 정보와 함께 출력 */
    private static void processTopic(String topic) {
        System.out.println(topic + "은(는) 쓰레드 " + Thread.currentThread().getName() + "(으)로 처리중...");
        try {
            Thread.sleep(10);
